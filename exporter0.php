<?php
	...
	class PDF extends tFPDF {
		...
		function PDF($orientation='P', $unit='mm', $size='A4') {
			...
			$this->tFPDF($orientation,$unit,$size); 								// Appel au constructeur parent
...
#		$pdf->SetFont('Helvetica','',20);
		$pdf->MultiCell(0,8,decodeIfNeeded($paper['title']),0,'C');
		$pdf->ln();
		//___________________________________
		$pdf->SetFont('DejaVuSans','',14);
		$authors = db_s('authors', array('paper_id' => $paper['id']), array('disp_order' => 'ASC'));
		while ($author = db_fetch($authors)) {
			$pdf->Cell(75, 7, decodeIfNeeded($author['last_name'].', '.$author['first_name']), 0, 1);
			$pdf->Cell(75, 7, decodeIfNeeded($author['affiliation']), 0, 1);
			$pdf->Link($pdf->getX(), $pdf->getY(), 75, 7, 'mailto:'.$author['email']);
			$pdf->Cell(75, 7, decodeIfNeeded($author['email']), 0, 1);
			$pdf->ln();
		}
		$pdf->ln();
...
// Seconde page
		$pdf->AddPage();
		$pdf->SetFontSize(12);
		$pdf->WriteHTML(decodeIfNeeded(str_replace('’', "'", html_entity_decode($paper['text']))));
		$pdf->WriteReferences();
		$pdf->Output();
	}
